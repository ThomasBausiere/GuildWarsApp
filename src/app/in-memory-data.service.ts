...
export class InMemoryDataService implements InMemoryDbService {
  ...
  // Méthode pour gérer les requêtes de recherche
  get(reqInfo: RequestInfo) {
    if (reqInfo.collectionName === 'skills') {
      const collection = reqInfo.collection as Skill[];
      const term = reqInfo.query.get('name')?.[0].toLowerCase();
      if (term) {
        const filteredCollection = collection.filter(skill => skill.name.toLowerCase().includes(term));
        return reqInfo.utils.createResponse$(() => {
          return {
            body: filteredCollection,
            status: 200,
          };
        });
      }
    }
    return undefined; // Laissez les autres requêtes non modifiées
  }
}
